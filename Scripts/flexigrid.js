...
(function ($) {
    $.addFlex = function (t, p) {
        ...
        p = $.extend({
            ...
            title: false, //是否包含标题
            pagestat: '从第 {from} 条显示到第 {to} 条记录，总记录数 {total} 条', //显示当前页和总页面的样式
        ...
        if (p.postColInfo) {
            if (!p.colkey) {
                ...
                for (var cindex = 0, clength = p.colModel.length; cindex < clength; cindex++) {
                    ...
                    if (p.colModel[cindex].summary) {
                        summaries.push(p.colModel[cindex].name + '$' + p.colModel[cindex].summary);
                    }
                    cols.push(p.colModel[cindex].name);
                }
                p.cols = cols.join(",");
                p.summaries = summaries.join(",");
            }
            if (!p.colkey) {
        ...
        var g = {
            ...
            addData: function (data) {
                ...
                if (p.summaryTemplate) {
                    $("#" + (p.id || 'grid') + '_footer').html($.tmpl(p.summaryTemplate, data.summary));
                }
                ...
                var cols = p.cols.split(",");
                tbhtml.push("<tbody>");
                var pindex = p.rp * (p.page - 1);
                if (p.dataType == 'json') {
                    if (data.rows != null) {
                        $.each(data.rows, function (i, row) {
                            g.addJsonRowData(tbhtml, ths, thsdivs, cols, pindex, row, i);
                            ...
                            //                                var div = [];
                            //                                div.push("<div style='text-align:", this.contentalign || this.align, ";width:", width, ";");
                            //                                if (p.nowrap == false) {
                            ...
                            //                                    if (this.process) {
                            //                                        divInner = this.process(divInner, trid, p.rows[row.id]);
                            //                                    }
                            //                                    div.push(divInner);
                            //                                }
                            //                                div.push("</div>");
            ...
            populate: function () { //get latest data 
                ...
                var param = [
                    ...
                    , { name: "colkey", value: p.colkey }
                    , { name: "colsinfo", value: p.cols }
                    , { name: "summary", value: p.summaries }
				];
            ...
            doSearch: function () {
                ...
                p.extParam = [{ name: "keywords", value: $('input[name=q]', g.tDiv).val()}];
                p.newp = 1;
                this.populate();
            },
            ...
            getSelectedRows: function () {
                ...
                $("tr.trSelected", g.bDiv).each(function () {
                    ...
                    items.push(g.getRow($(this).attr("id")));
                });
                return items;
            },
            ...
            addJsonRowData: function (tbhtml, ths, thsdivs, cols, pindex, row, i) {
                // 组装Json格式的数据
                if (row.id && !p.rows[row.id]) {
                    var entity = {};
                    for (var k = 0; k < cols.length; k++) {
                        if (!entity[cols[k]]) {
                            entity[cols[k]] = row.cell[k];
            ...
            appendRowData: function (data) {
                var tbhtml = [];
                var theads = $('thead tr:first th', g.hDiv);
                var theaddivs = $('thead tr:first th div', g.hDiv);
                var pindex = p.rp * (p.page - 1);
                var cols = p.cols.split(",");
                var key = p.colkey;
                var row = {};
                ...
                row["cell"] = [];
                for (var k = 0; k < cols.length; k++) {
                    row.cell.push(data[cols[k]]);
                }

                g.addJsonRowData(tbhtml, theads, theaddivs, cols, pindex, row, $("tbody tr", t).length);
                if (t.innerHTML == '') {
                    $(t).html("<tbody></tbody>");
                }
                $("tbody", t).append(tbhtml.join(""));
            },
            getRow: function (key) {
                ...
                if (p.rows) {
                    var row = p.rows[key];
                    var ctrls = $("#row" + key, t).find("input,select");
                    $.each(ctrls, function () {
                        var self = $(this);
        ...
        if (p.colModel) {
            ...
            for (i = 0; i < p.colModel.length; i++) {
                ...
                if (cm.align)
                    th.align = cm.align;
                if (cm.contentalign) {
                    th.contentalign = cm.contentalign;
                }
                if (cm.width)
        ...
        g.sDiv = document.createElement('div'); // 创建footer（E2home 2012-08-02）
        ...
        if (p.buttons) {
                ...
                if (!btn.separator) {
                    ...
                    if (btn.onpress) {
                        $(btnDiv).click
							(
								function () {
								    this.onpress(this.name, this.btn, g.gDiv);
								}
							);
                    }
                    $(tDiv2).append(btnDiv);
            ...
            var $tDiv = $(g.tDiv).append(tDiv2);

            //add search button
            if (p.qtype != '') {
                // 第一种检索方式：快速检索
                $tDiv.append("<div class='queryArea'><input type='text' name='q' class='querytext'/><a href='javascript:void(0);' name='btnClearQuery' style='display: none'><span class='clearquery'>&nbsp; </span></a><a href='javascript:void(0);' name='btnQuickQuery'><span class='usequery'>&nbsp;</span></a></div>");
                $('input[name=q]', g.tDiv).val(p.searchtext).attr("alt", p.searchtext)
                    .focus(function (e) { // 1.当获得焦点时 隐藏水印
                    ...
                    .keydown(function (e) { // 3.在输入框中按Enter同点击查询按钮
                        if (e.keyCode == 13) {
                            g.doSearch();
                            if (this.value != "") {
                                $('a[name=btnClearQuery]', g.tDiv).show();
                                $('a[name=btnQuickQuery]', g.tDiv).hide();
                            }
                            else {
                                $('a[name=btnQuickQuery]', g.tDiv).show();
                                $('a[name=btnClearQuery]', g.tDiv).hide();
                            }
                        }
                    });

                // 4.点击快速检索按钮查询数据
                $('a[name=btnQuickQuery]', g.tDiv).click(function (e) {
                    g.doSearch();
                    if ($('input[name=q]', g.tDiv).val() != "") {
                        $('a[name=btnClearQuery]', g.tDiv).show();
                        $('a[name=btnQuickQuery]', g.tDiv).hide();
                    }
                    else {
                ...
                // 5.点击清空快速检索按钮的查询所有数据
                $('a[name=btnClearQuery]', g.tDiv).click(function (e) {
                ...
                if (p.searchitems.length > 0) {
                    $tDiv.find("div.queryArea").append("<div id='advquerybutton' class='advquerybutton'><div class='fbutton' title='展开高级检索区域'><div><span class='querydown'>高级检索</span></div></div></div>");
                    // 点击高级检索按钮，则显示层
                    $("div.advquerybutton", g.tDiv).click(function (e) {
                        $("#advQueryPanel").slideToggle(200);
                        $('input[name=q]', g.tDiv).val("");
                        $('a[name=btnQuickQuery]', g.tDiv).show();
                        $('a[name=btnClearQuery]', g.tDiv).hide();
                        $('input[name=q]', g.tDiv).val(p.searchtext).attr("alt", p.searchtext)
                    ...
                    $tDiv.find("div.queryArea").width(212);
                }
            }
        ...
        if (p.usepager) {
            ...
            $(g.bDiv).after(g.pDiv);
            var html = '<div class="pGroup"><div class="pFirst pButton" title="转到第一页"><span></span></div><div class="pPrev pButton" title="转到上一页"><span></span></div> </div><div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">第 <input type="text" style="width:20px" value="1" /> 页，共 <span> 1 </span> 页</span></div><div class="btnseparator"></div><div class="pGroup"> <div class="pNext pButton" title="转到下一页"><span></span></div><div class="pLast pButton" title="转到最后一页"><span></span></div></div><div class="btnseparator"></div><div class="pGroup"> <div class="pReload pButton" title="刷新"><span></span></div> </div> <div class="btnseparator"></div><div class="pGroup"><span class="pPageStat"></span></div>';
        ...
        //add footer，sDiv原为快速搜素区域，现在改造为底部 （E2home 2012-08-02）
        if (p.showFooter) {
            //add footer box
            g.sDiv.className = 'sDiv';
            var footerId = (p.id || 'grid') + '_footer';
            $(g.sDiv).append("<div id='" + footerId + "' class='sDiv2 footer'></div>");
            $(g.bDiv).after(g.sDiv);
        }
    ...
    // 获取指定Key的数据行（json）
    $.fn.flexGetRowData = function (key) {
        if (this[0].grid) {
            return this[0].grid.getRow(key);
        }
        return null;
    };
    // 获取表格的所有数据（json）
    $.fn.flexGetData = function () {
        ...
        var tb = this[0];
        if (tb.grid) {
            for (var key in tb.p.rows) {
                data.push(tb.grid.getRow(key));
            }
            return data;
        }
        return [];
    ...
    // 在客户端增加一行或多行数据
    $.fn.flexAddRows = function (data) {
        return this.each(function () {
            if (this.grid) {
                if ($.isArray(data)) {
                    for (var i = 0; i < data.length; i++) {
                        this.grid.appendRowData(data[i]);
                    }
                }
                else if (data) {
                    this.grid.appendRowData(data);
                }
            }
    ...
    // 删除指定Key的数据行
    $.fn.flexDeleteRowData = function (key) {
        return this.each(function () {
            if (this.grid) {
